...
"<button class='btn btn-secondary' onclick='exportSalesCsv()'>&#8595; CSVダウンロード</button>"
"</div></div>\n"
...
"\n"
"<!-- PLAN IMAGES MODAL -->\n"
"<div class='overlay' id='plan-images-modal'>\n"
"  <div class='modal' style='max-width:560px;'>\n"
"    <h3>プラン画像管理 <span id='pimg-plan-id-label' style='color:var(--muted);font-size:13px;'></span></h3>\n"
"    <div id='pimg-msg'></div>\n"
"    <input type='hidden' id='pimg-plan-id'>\n"
"    <div id='pimg-list' style='margin-bottom:16px;'></div>\n"
"    <div style='background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:12px;'>\n"
"      <div class='form-group' style='margin-bottom:8px;'>\n"
"        <label>画像URL *</label>\n"
"        <input id='pimg-url' type='url' placeholder='https://example.com/image.jpg'>\n"
"      </div>\n"
"      <div class='form-group' style='margin-bottom:0;'>\n"
"        <label>表示順（数値）</label>\n"
"        <input id='pimg-order' type='number' value='0' min='0'>\n"
"      </div>\n"
"      <button class='btn btn-primary' style='width:100%;margin-top:10px;' onclick='addPlanImage()'>+ 追加</button>\n"
"    </div>\n"
"    <div class='modal-footer'>\n"
"      <button class='btn btn-secondary' style='width:100%;' onclick='closeModal(\"plan-images-modal\")'>閉じる</button>\n"
"    </div>\n"
"  </div>\n"
"</div>\n"
"\n"
...
"       <button class='btn btn-secondary' onclick='openPlanImages(${p.id})'>&#128444; 画像</button>\n"
...
"\n"
"// ─── Plan Images ────────────────────────────────────────────────────────────\n"
"async function openPlanImages(planId) {\n"
"  document.getElementById('pimg-plan-id').value = planId;\n"
"  document.getElementById('pimg-plan-id-label').textContent = '(Plan ID: '+planId+')';\n"
"  document.getElementById('pimg-url').value = '';\n"
"  document.getElementById('pimg-order').value = '0';\n"
"  showMsg('pimg-msg','',false);\n"
"  openModal('plan-images-modal');\n"
"  await loadPlanImages(planId);\n"
"}\n"
"async function loadPlanImages(planId) {\n"
"  const r = await api('GET', `/api/v1/admin/plans/${planId}/images`);\n"
"  const el = document.getElementById('pimg-list');\n"
"  if (!r.ok) { el.innerHTML = '<p style=\"color:var(--danger)\">読み込みエラー</p>'; return; }\n"
"  const imgs = r.data.images || [];\n"
"  if (!imgs.length) { el.innerHTML = '<p style=\"color:var(--muted);font-size:13px;\">画像なし</p>'; return; }\n"
"  el.innerHTML = imgs.map(img =>\n"
"    `<div style='display:flex;align-items:center;gap:10px;margin-bottom:8px;'>`\n"
"    +`<img src='${img.url}' style='width:64px;height:48px;object-fit:cover;border-radius:4px;border:1px solid var(--border);' onerror='this.style.display=\"none\"'>`\n"
"    +`<div style='flex:1;font-size:12px;color:var(--muted);word-break:break-all;'>${img.url}</div>`\n"
"    +`<span style='font-size:11px;color:var(--sub);'>順: ${img.display_order}</span>`\n"
"    +`<button class='btn btn-danger btn-sm' onclick='deletePlanImage(${img.id},${planId})'>削除</button>`\n"
"    +`</div>`\n"
"  ).join('');\n"
"}\n"
"async function addPlanImage() {\n"
"  const planId = document.getElementById('pimg-plan-id').value;\n"
"  const url    = document.getElementById('pimg-url').value.trim();\n"
"  const order  = parseInt(document.getElementById('pimg-order').value) || 0;\n"
"  if (!url) { showMsg('pimg-msg', false, 'URLを入力してください'); return; }\n"
"  const r = await api('POST', `/api/v1/admin/plans/${planId}/images`, { url, display_order: order });\n"
"  if (r.ok) {\n"
"    document.getElementById('pimg-url').value = '';\n"
"    showMsg('pimg-msg', true, '追加しました');\n"
"    await loadPlanImages(planId);\n"
"  } else {\n"
"    showMsg('pimg-msg', false, r.data?.error || 'エラー');\n"
"  }\n"
"}\n"
"async function deletePlanImage(imgId, planId) {\n"
"  if (!confirm('この画像を削除しますか？')) return;\n"
"  const r = await api('DELETE', `/api/v1/admin/plan-images/${imgId}`);\n"
"  if (r.ok) { showMsg('pimg-msg', true, '削除しました'); await loadPlanImages(planId); }\n"
"  else showMsg('pimg-msg', false, r.data?.error || 'エラー');\n"
"}\n"
"\n"
"// ─── CSV Export ──────────────────────────────────────────────────────────────\n"
...
"  const to     = toEl?.value   || '';\n"
"  const r = await api('GET', `/api/v1/admin/reports/sales?from=${from}&to=${to}`);\n"
